...
def run_test(command, input_file, output_file, dev_mode=False):
    ...
    if dev_mode:
        ...
        if not check_command_available(command):
            ...
            print(f"  Or run this script in development mode: python test_installation.py --dev")
            return False
        cmd_list = [command]
        cmd_display = command
...
def find_examples_directory():
    ...
    search_paths = [
        ...
        (os.path.join(script_dir, 'commandline', 'circuit_json_examples'), "circuit examples directory"),
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if test1_ok and test2_ok and test3_ok:
            ...
            if not check_command_available("generate_sizes_probe_subckt"):
                print("\nSolution: The package doesn't appear to be installed properly.")
                print("To install the package in development mode (from the source directory):")
                print("    pip install -e .")
